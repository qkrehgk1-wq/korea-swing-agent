...
> 차트·거래량·이동평균·패턴·수급·공식 재무(DART)를 근거로, 5개 역할의 LLM 에이전트가 한국 주식의 단기~중기(2~8주) 스윙 타점을 분석합니다. 미국 주식은 Alpaca 읽기 전용 백테스트와 통합 전략 검증을 지원합니다.
...
### 8. Billionaire Core v1 통합 전략
기존 Signal Council, 기술적 컨플루언스, Stoch–ADX–ATR, ATR 손실 관리, R 기대값, 시장 레짐, 진화 학습을 하나의 결정론 엔진으로 묶었습니다. 뉴스·수급·LLM은 역사적 점수에 섞지 않고 라이브 위험 경고로만 사용하며, 라이브 기대값이 양수가 아니면 자동 사이징을 막습니다.
- 실행: `corepack pnpm start:swing:billionaire`
- 명세: `data/strategy/billionaire-core-v1.md`
- 결과: `.data/backtests/billionaire-core-v1.md`

### 9. 시세 정확성 검수 에이전트
텔레그램 발송 전에 티커·종목명·최신 종가·시세 날짜·현재가 차이·기준가/주의가격 관계·OHLCV 이상 여부를 다시 확인합니다. 검수 실패 종목은 발송과 저널 기록에서 제외하고, 결과는 `.data/quality/market-data/latest-report.md`에 남깁니다.
새 저널 기록에는 당시 `currentPrice`와 시장을 저장하며, 과거 기록 감사는 현재 시세가 아니라 기록 시각(KST) 이전의 참조 거래일 OHLCV와 비교합니다. 소급 감사는 `npm run start:swing:accuracy`로 실행하고 `.data/quality/market-data/journal-audit.md`에 별도로 남깁니다.
라이브 기대값이 음수인 동안에는 후보를 관찰용으로만 표시하고 텔레그램에 `실거래 금지 · 현재는 관찰용` 경고를 자동 표시합니다.
저널에는 `scoringVersion`을 함께 저장해 진입 당일을 포함하는 백테스트 정산 규칙과 라이브 결과의 일치 여부를 추적합니다. 기존 기록은 원본을 보존한 채 `npm run start:swing:journal-audit`로 재계산 비교합니다.
